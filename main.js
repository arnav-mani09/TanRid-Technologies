...
const buildMailLink = (to, subject, body) => {
  const encodedSubject = encodeURIComponent(subject);
  const encodedBody = encodeURIComponent(body);
  return `mailto:${to}?subject=${encodedSubject}&body=${encodedBody}`;
};
document.querySelectorAll("[data-mail-to]").forEach(button => {
  button.addEventListener("click", () => {
...
const downloadButton = document.querySelector("[data-download='latest-brief']");
...
const hideCookieBanner = () => cookieBanner?.classList.remove("visible");
